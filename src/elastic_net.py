...
class ElasticNetIC:
    ...
    def __init__(
        ...
        l1_ratios : array-like, optional
            List of l1_ratio values to try (between 0 and 1). If None, uses a simple linear range.
        criterion : {'aic', 'bic'}
        ...
        if l1_ratios is None:
            l1_ratios = [0.1, 0.5, 0.75, 0.9, 0.95, 0.99]  # NOTE as suggested by chat
    ...
    def fit(self, X, y):
        """
        Fit an ElasticNet model for each (alpha, l1_ratio) in the grid on the entire dataset,
    ...
    def get_ic_results(self):
        """
        Retrieve the full grid of (alpha, l1_ratio, df, rss, IC) results.

        Returns
        -------
        results : list of dict
            Each dict has keys ['alpha', 'l1_ratio', 'df', 'rss', <IC name>].
        """
